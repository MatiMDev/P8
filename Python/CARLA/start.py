...
def main():
    ...
    def process_depth(image):
        ...
        rgb_depth = array[:, :, :3].copy()  # Use raw RGB depth (do not convert!)

        # Decode depth for visualization
        ...
        depth_meters = norm_depth * 1000.0  # 0-80m typical range
        ...
        # === Send to grid mapper ===
        if segm_array is not None:
            segm_mask = np.isin(segm_array, OBSTACLE_CLASS_IDS)
    ...
    try:
        while True:
            ...
            vehicle.apply_control(carla.VehicleControl(throttle=0.1))
            ...
            occ_surface = pygame.transform.scale(occ_surface, (800, 600))  # Resize if needed
            display.blit(occ_surface, (1600, 600))  # Adjust placement as needed
