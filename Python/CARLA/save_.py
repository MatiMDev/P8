...
OBSTACLE_CLASS_IDS = [
    3, 4, 5,       # Building, Wall, Fence
    6, 7,          # Pole, Traffic Light
    12, 13,        # Pedestrian, Rider
    14, 15, 16, 17, 18, 19  # Vehicles and bikes
]
...
def main():
    ...
    display = pygame.display.set_mode((1600, 1200), pygame.HWSURFACE | pygame.DOUBLEBUF)
    ...
    def process_depth(image):
        ...
        if depth_mode == 0:
            ...
            depth_surface = to_surface(image)
        elif depth_mode == 1:
            image.convert(carla.ColorConverter.Depth)
            depth_surface = to_surface(image)
        elif depth_mode == 2:
            depth_surface = to_surface(image)  # Raw for debugging
        elif depth_mode == 3:
            image.convert(carla.ColorConverter.Depth)
            ...
            depth_surface = pygame.surfarray.make_surface(heatmap.swapaxes(0, 1))
